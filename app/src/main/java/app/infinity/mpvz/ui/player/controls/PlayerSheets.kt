...
import app.infinity.mpvz.ui.player.controls.components.sheets.ExternalAudioSourceDialog
import app.infinity.mpvz.ui.player.controls.components.sheets.ExternalAudioUrlDialog
  ...
  when (sheetShown) {
    ...
    Sheets.AudioTracks -> {
      ...
      if (showAudioUrlDialog) {
        ExternalAudioUrlDialog(
          onConfirm = { url ->
            showAudioUrlDialog = false
            onAddAudio(Uri.parse(url))
          },
      ...
      AudioTracksSheet(
        ...
        onAddAudioTrack = { showAudioSourceDialog = true },
        onOpenDelayPanel = { onOpenPanel(Panels.AudioDelay) },
        onOpenEqualizerSheet = { onShowSheet(Sheets.Equalizer) },
        externalAudioEnabled = activeEngine == PlaybackEngineMode.MPV,
